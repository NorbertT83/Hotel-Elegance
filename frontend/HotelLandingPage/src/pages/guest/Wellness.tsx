...
export default function Wellness() {
    ...
    const wellnessServices = useMemo(() => {
        return services
            .filter((service) => service.service_type_en === 'Wellness')
            .sort((a, b) => {
    ...
    function handleCartChange(id: HotelService['id'], quantity: number) {
        setCart((prev) => {
            ...
            const serviceItem = wellnessServices.find((service) => String(service.id) === idString);
    ...
    async function placeOrder(cartValue: number) {
        try {
            ...
            console.error('Hiba történt a wellness rendelés elküldése során:', err?.message || err);
        }
    }
