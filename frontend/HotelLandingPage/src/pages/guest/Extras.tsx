...
export default function Extras() {
    ...
    const labels = guestPageText[language].guestPage.menuExtras;
    const { currentBooking, services, refreshBookedServices } = useGuest();
    const [cart, setCart] = useLocalStorageState<ServiceOrderItem[]>('guest-cart-extras', []);
    const [isCartOpen, setIsCartOpen] = useState<boolean>(false);
    const [showSuccessModal, setShowSuccessModal] = useState<boolean>(false);

    const extrasServices = useMemo(() => {
        return services
            .filter((service) => service.service_type_en === 'Extras')
            .sort((a, b) => {
    ...
    function handleCartChange(id: HotelService['id'], quantity: number) {
        setCart((prev) => {
            ...
            const serviceItem = extrasServices.find((service) => String(service.id) === idString);
    ...
    async function placeOrder(cartValue: number) {
        try {
            ...
            console.error('Hiba történt az extra szolgáltatás rendelése közben:', err?.message || err);
        }
    }
