...
export default function Step1BookingDetails() {
    const { arrivalDate, departureDate, guests, nextStep } = useBooking();
        ...
        <div className={s.cardContainer}>
            <div className={s.card}>
                ...
                <div className={s.bookingDetails}>
                    <p><span>{labels.arrival}:</span><span>{arrivalDate}. - {getNameOfDay(arrivalDate, language)}</span></p>
                    <p><span>{labels.departure}:</span><span>{departureDate}. - {getNameOfDay(departureDate, language)}</span></p>
                    <p><span>{labels.adults}:</span><span>{guests.adult} {labels.person}</span></p>
                    <p><span>{labels.children}:</span><span>{guests.child} {labels.person}</span></p>
                </div>
